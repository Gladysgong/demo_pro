...
def get_menu_html(menu_data):
    ...
    option_str = """
             <li class="treeview {display}">
            <a class="app-menu__item" href="#" data-toggle="treeview">
                <i class="app-menu__icon fa {icon_random}"></i>
                <span class="app-menu__label">{menu_title}</span>
    ...
    # """
    url_str = """
            <li><a class="treeview-item {active}" href="{permission_url}"><i class="icon fa fa-circle-o"></i>{permission_title}</a></li>
        """
        ...
        if not item['status']: # 如果用户权限不在某个菜单下，即item['status']=False, 不显示
            ...
            if item.get('url'): # 说明循环到了菜单最里层的url
                ...
                menu_html += option_str.format(menu_title=item['title'],
                                               sub_menu=sub_menu,
                                               # display="" if item['open'] else "is-expanded",
